...
function build(absProjectPath) {
    ...
    if (config.theme.pkg) {
        ...
        themeDir = (0, path_1.resolve)(rootDir, config.theme.path);
    }
    if (!themeDir) {
        (0, logger_1.error)(`Please specify a theme directory or package name but got: ${config.theme}`);
        return;
    }
    // 数据源 数据源引用资源 输出目录
    ...
    rd_1.default.eachFileFilterSync(srcPostsDir, /\.md$/, (f) => {
        ...
        if (!mdDescriptor) {
            (0, logger_1.error)('[FAILED] post compile failed: ', f);
            return;
        }
        ...
        if (filterCategories.length !== mdDescriptor.categories.length) {
            (0, logger_1.warn)(`[FILTER] Post not compiled into Index page by filter [${(_b = config.settings.homepage) === null || _b === void 0 ? void 0 : _b.filter}]: `, f);
        }
        else {
